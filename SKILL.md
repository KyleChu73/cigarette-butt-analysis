...
description: 静态价值型烟蒂股量化分析技能（v1.6）。基于三大支柱（存量资产垫、低维持运营开支、资产兑现逻辑）和 Fact Check 21 项验证体系，对标的进行系统性分析。核心更新：(1) 修复 T2 公式遗漏（短期理财/定期存款纳入）；(2) 加分体系（#20/#21）统一定义、各子类型引用；(3) B 型筛选条件明确"持股比例≥10%"与"覆盖率≥30%"为两个独立必要条件；(4) C2 评分新增"估值安全垫≥1 分"硬性门槛；(5) 治理评分去除无效权重列；(6) 补充边界条件（负净资产、双重股权、币种换算、财年对齐、中报年化）；(7) 新增 [COMPUTABLE]/[LLM-REQUIRED] 标签体系；(8) 与 QuantRadar 平台监控频率对齐。使用场景：用户上传财报（PDF 或文字表格）并提供股票代码/名称时触发；支持港股/A 股/美股；强制联网搜索最新市场数据并记录来源 URL；输出包含 Executive Summary、商业模式分析、管理层与治理分析、三大支柱评估、子类型判定（A 高股息/B 套利/C 事件驱动）、Fact Check 验证、操作建议（入场价/止盈/止损/收益周期计算）、风险提示的完整 Markdown 报告。
---
...
### Step 5: 三大支柱评估
- **支柱一** `[COMPUTABLE]`：计算 T0_NAV、T1_NAV、T2_NAV（**[v1.6]** T2 公式纳入短期理财 + 定期存款），确定分级
...
- **B 型** `[COMPUTABLE]`：SOTP 估值表 + 折价分解 + 三情景敏感性分析
  - **核心条件（[v1.6] 明确为 4 项）**：控股折价率≥30%、至少一家子公司持股比例≥10%、持股价值覆盖率≥30%、母公司净现金>0（四项全部满足才成立）
- **C1 型** `[LLM-REQUIRED]`：事件概率 - 收益矩阵 + 时间线分析
- **C2 型** `[LLM-REQUIRED]`：准入条件 4 项验证 + 评分体系（4 维度 10 分，**[v1.6] 估值安全垫≥1 分为硬性门槛**）
...
- #20-#21 加分项核查（**[v1.6] 规则见支柱二加分体系统一定义**）
...
- **[v1.6] 边界条件处理**：
  ...
  - **中报年化**：中报计算 FCF/烧损率等年化指标时标注"年化（基于中报×2）"及局限性
...
9. Fact Check 21 项验证 — #1-#19 核心检查（**含警告分类**）、#20-#21 加分（**规则见支柱二**）、综合评级
